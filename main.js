...
document.querySelectorAll('a[href^="#"]').forEach(link => {
  link.addEventListener('click', e => {
    ...
    const annualTotals = { starter: 192, pro: 588, ent: 1368 };
    let isAnnual = false;
    ...
    function initPricingToggle() {
      ...
      if (!pricingToggle) {
        console.error('pricingToggle element not found');
        return;
      }
      ...
      function updatePricing() {
        ...
        if (starterEl) starterEl.textContent = prices.starter[idx];
        if (proEl) proEl.textContent = prices.pro[idx];
        if (entEl) entEl.textContent = prices.ent[idx];
        const starterNoteEl = document.getElementById('annual-note-starter');
        const proNoteEl = document.getElementById('annual-note-pro');
        const entNoteEl = document.getElementById('annual-note-ent');
        if (starterNoteEl) starterNoteEl.textContent = isAnnual ? `$${annualTotals.starter} billed annually` : '\u00a0';
        if (proNoteEl) proNoteEl.textContent = isAnnual ? `$${annualTotals.pro} billed annually` : '\u00a0';
        if (entNoteEl) entNoteEl.textContent = isAnnual ? `$${annualTotals.ent} billed annually` : '\u00a0';
      ...
      pricingToggle.addEventListener('click', (e) => { 
        ...
        isAnnual = !isAnnual; 
        console.log('New isAnnual:', isAnnual);
        updatePricing();
        console.log('Toggle class after update:', pricingToggle.className);
      });
